...
class User{
    ...
    async findEmail(email) {
        try {
            ...
            if(result.length > 0){ // Se o email passado não for igual o email no DB, retorna 0 no result
                return true
            }else{
                return false
            }
            ...
            if(email != undefined) {
                if(email != user.email){
                    ...
                    if(result == false){
                        ...
                        return{status: false,err: "Email already taken"}
                    }
                }
            }
    ...
    async delete(id){
        ...
        if(user != undefined) {
            ...
            return {status: false, err: "User does not exist, so can't be deleted"}
        }
    } 
